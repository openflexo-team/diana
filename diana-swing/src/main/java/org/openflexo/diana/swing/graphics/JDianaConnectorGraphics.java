...
public class JDianaConnectorGraphics extends JDianaGraphics implements DianaConnectorGraphics {

	private static final Logger logger = Logger.getLogger(JDianaConnectorGraphics.class.getPackage().getName());

	private final JDianaSymbolGraphics symbolGraphics;

	public <O> JDianaConnectorGraphics(ConnectorNode<O> node, JConnectorView<O> view) {
		super(node, view);
		symbolGraphics = new JDianaSymbolGraphics(node, view);
	}
	...
	@Override
	public JDianaSymbolGraphics getSymbolGraphics() {
	 ...
	 */
	@Override
	public void drawSymbol(DianaPoint point, ConnectorSymbol symbol, double size, double angle) {
	...
	public void drawSymbol(double x, double y, ConnectorSymbol symbol, double size, double angle) {
		...
		DianaArea symbolShape = symbol.getSymbol();

		// Debug: to see bounds
		// symbolShape = new DianaUnionArea(symbolShape,new DianaRectangle(0,0,1,1,Filling.NOT_FILLED));
