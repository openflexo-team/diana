...
import org.openflexo.diana.vaadin.client.CanvasClientRpc;
import org.openflexo.diana.vaadin.client.CanvasServerRpc;

import com.vaadin.shared.MouseEventDetails;

@SuppressWarnings("serial")
public class Canvas extends com.vaadin.ui.AbstractComponent {
	private final ArrayList<CanvasClickListener> clickListeners = new ArrayList<CanvasClickListener>();
	private final ArrayList<CanvasPressListener> pressListeners = new ArrayList<CanvasPressListener>();
	private final ArrayList<CanvasImageLoadListener> imageLoadListeners = new ArrayList<CanvasImageLoadListener>();
	 ...
	 */
	public void drawImage2(String url, double offsetX, double offsetY, double imgWidth, double imgHeight) {
	 ...
	 *            URL of the image
	 * @param sourceX
	 *            the X coordinate of the upper left corner of the area in the source image to draw
	 * @param sourceY
	 *            the Y coordinate of the upper left corner of the area in the source image to draw
	 * @param sourceWidth
	 ...
	 *            the height of the drawing area
	 */
	public void drawImage3(String url, double sourceX, double sourceY, double sourceWidth, double sourceHeight, double destX, double destY,
			double destWidth, double destHeight) {
		rpc.drawImage3(url, sourceX, sourceY, sourceWidth, sourceHeight, destX, destY, destWidth, destHeight);
	}

	/**
	 * Dessine un Rectangle plein en prenant en argument un objet de type Rectangle.
	 * 
	 * @param rect
	 */
	...
	/**
	 * Set the current font properties for text content on the canvas. Uses the same syntax as the CSS font property.
	 * 
	 * @param font
	 *            the new font
	 */
	...
	/**
	 * Sets the text baseline used when drawing text. Possible values are: "top", "hanging", "middle", "alphabetic", "ideographic" or
	 * "bottom". Default value is "alphabetic".
	 * 
	 * The {@link #fillText(String, double, double, double)} will use the specified textBaseline value when positioning the text on the
	 * canvas.
	 * 
	 * @param textBaseline
	...
	/**
	 * Add a a new point and creates a line from that point to the last specified point in the canvas (this method does not draw the line).
	 * Use the
	 * 
	 * @param x
	 *            the x coordinate where to create a line to
	 * @param y
	 *            the y coordinate where to create a line to {@link #stroke()} method to actually draw the path on the canvas.
	 */
	...
	/**
	 * Adds a point to the current path by using the specified control points that represent a quadratic B斯ier curve.
	 * 
	 * A quadratic B斯ier curve requires two points. The first point is a control point that is used in the quadratic B斯ier calculation and
	 * the second point is the ending point for the curve. The starting point for the curve is the last point in the current path. If a path
	 * does not exist, use the {@link #beginPath()} and {@link #moveTo(double, double)} methods to define a starting point.
	 * 
	 * @param cpx
	 *            The X coordinate of the B斯ier control point
	 * @param cpy
	 *            The Y coordinate of the B斯ier control point
	 * @param x
	 ...
	 * @param height
	 *            the height of the Rectangle {@link #fill()} methods to actually draw the Rectangle on the canvas.
	 */
	...
	/**
	 * Rotates the current drawing. The rotation will only affect drawings made AFTER the rotation is done.
	 * 
	 * @param angle
	...
	/**
	 * Sets the style of the end caps for a line. Possible values are "butt", "round" and "square".
	 * 
	...
	/**
	 * Sets the type of corner created, when two lines meet. Possible values are "bevel", "round" and "miter".
	 * 
	 * The "miter" value is affected by the miterLimit property, which you can set using the {@link #setMiterLimit(double)} method.
	 * 
	 * @param lineJoin
	 *            the new line join
	 */
	 ...
	 * 
	 * The miter length grows bigger as the angle of the corner gets smaller. To prevent the miter length from being too long, we can use
	 ...
	 * 
	 * Use the {@link #setStrokeStyle(String)} method to set a color, gradient, or pattern to style the stroke.
	 * 
	 * @param startX
	 *            the X coordinate of the upper left corner of the Rectangle
	 * @param startY
	 ...
	 * 
	 * This method replaces the current transformation matrix. It multiplies the current transformation matrix with the matrix described by:
	 * 
	 * a c e
	 * 
	 * b d f
	 ...
	 * 0 0 1
	 * 
	 * In other words, the it lets you scale, rotate, move, and skew the current context.
	 * 
	 * The transformation will only affect drawings made after the method is called.
	 * 
	 * The {@link #transform(double, double, double, double, double, double)} method behaves relatively to other transformations made by
	 * 
	 * @param m11
	 ...
	 * @param dy
	 *            Moves the the drawing vertically {@link #rotate(double)}, {@link #scale(double, double)} or
	 *            {@link #translate(double, double)}.
	 * 
	 *            For example: If you already have set your drawing to scale by two, and the
	 *            {@link #transform(double, double, double, double, double, double)} method scales your drawings by two, your drawings will
	 *            now scale by four.
	 */
	...
	/**
	 * This method creates an arc/curve (used to create circles, or parts of circles). To create a circle with arc(): Set start angle to 0
	 * and end angle to 2 * Math.PI.
	 * 
	 * Use the {@link #stroke()} or {@link #fill()} method to actually draw the arc on the canvas.
	 * 
	 * @param x
	...
	/**
	 * This method remaps the (0,0) position on the canvas. When you call a method such as {@link #fillRect(double, double, double, double)}
	 * after
	 * 
	 * @param x
	 *            The value to add to horizontal (x) coordinates
	 * @param y
	 *            The value to add to vertical (y) coordinates {@link #translate(double, double)}, the value is added to the x- and
	 *            y-coordinate values.
	 */
	 ...
	 * 
	 * If you scale a drawing, all future drawings will also be scaled. The positioning will also be scaled. For example, if you scale(2,2);
	...
	/**
	 * Draw the path you have defined with eg {@link #moveTo(double, double)} and {@link #lineTo(double, double)} methods.
	 * 
	 * The default color is black. Use the {@link #setStrokeStyle(String)} to draw with another color/gradient.
	 */
	public void stroke() {
		rpc.stroke();
	}
	 ...
	 * 
	 * Use {@link #moveTo(double, double)}, {@link #lineTo(double, double)}, {@link #quadraticCurveTo(double, double, double, double)} etc.
	 * and arc(), to create paths.
	 * 
	 * Use the {@link #stroke()} method to actually draw the path on the canvas.
	 */
	 ...
	 * 
	 * The alpha value must be a number between 0.0 (fully transparent) and 1.0 (no transparancy).
	 * 
	 * @param alpha
	 ...
	 * Use the {@link #stroke()} method to actually draw the path on the canvas.
	 * 
	 * Use the {@link #fill()} method to fill the drawing (black is default). Use the {@link #setFillStyle(String)} method to fill with
	...
	/**
	 * Sets how a source (new) image are drawn onto a destination (existing) image.
	 * 
	 ...
	 * 
	 * "source-atop" Displays the source image on top of the destination image. The part of the source image that is outside the destination
	 * image is not shown
	 * 
	 * "source-in" Displays the source image in to the destination image. Only the part of the source image that is INSIDE the destination
	 * image is shown, and the destination image is transparent
	 * 
	 * "source-out" Displays the source image out of the destination image. Only the part of the source image that is OUTSIDE the
	 ...
	 * 
	 * destination-atop Displays the destination image on top of the source image. The part of the destination image that is outside the
	 * source image is not shown
	 * 
	 * "destination-in" Displays the destination image in to the source image. Only the part of the destination image that is INSIDE the
	 * source image is shown, and the source image is transparent
	 * 
	 * "destination-out" Displays the destination image out of the source image. Only the part of the destination image that is OUTSIDE the
	 * source image is shown, and the source image is transparent
	 * 
	...
	/**
	 * Creates a linear gradient object with the specified name The gradient can be used to fill Rectangles, circles, lines, text, etc.
	 * 
	 * Use the gradient name as a parameter for {@link #setStrokeStyle(String)} or {@link #setFillStyle(String)}.
	 * 
	 * Use the {@link #addColorStop(String, double, String)} method to specify different colors, and where to position the colors in the
	 * gradient object.
	 * 
	 * @param name
	 *            the gradient name
	...
	/**
	 * Creates a radial gradient object with the specified name The gradient can be used to fill Rectangles, circles, lines, text, etc.
	 * 
	 * Use the gradient name as a parameter for {@link #setStrokeStyle(String)} or {@link #setFillStyle(String)}.
	 * 
	 * Use the {@link #addColorStop(String, double, String)} method to specify different colors, and where to position the colors in the
	...
	/**
	 * Specifies the colors and position in a gradient object. It is used together with
	 * 
	 * @param gradient
	 *            the gradient name
	 * @param offset
	 ...
	 * @param color
	 *            A CSS color value to display at the stop position {@link #createLinearGradient(String, double, double, double, double)} or
	 *            {@link #createRadialGradient(String, double, double, double, double, double, double)} .
	 * 
	 *            Note: You can call this method multiple times to change a gradient. If you omit this method for gradient objects, the
	 *            gradient will not be visible. You need to create at least one color stop to have a visible gradient.
	 */
	...
	/**
	 * The listener interface for receiving canvasClick events. The class that is interested in processing a canvasClick event implements
	 * this interface, and the object created with that class is registered with a component using the component's
	 * <code>addCanvasClickListener<code> method. When the canvasClick event occurs, that object's appropriate method is invoked.
	 * 
	...
	public interface CanvasPressListener {
	 ...
	 *            the listener
	 */
	public void addListener(CanvasPressListener listener) {
	 ...
	 *            the listener
	 */
	public void removeListener(CanvasPressListener listener) {
		if (pressListeners.contains(listener)) {
	...
	private void firePressed(MouseEventDetails med) {
		for (CanvasPressListener listener : pressListeners) {
			listener.pressed(med);
		}
	}

	/**
	 * The listener interface for receiving canvasImageLoad events. The class that is interested in processing a canvasImageLoad event
	 * implements this interface, and the object created with that class is registered with a component using the component's
	 * <code>addCanvasImageLoadListener<code> method. When the canvasImageLoad event occurs, that object's appropriate method is invoked.
	 * 
