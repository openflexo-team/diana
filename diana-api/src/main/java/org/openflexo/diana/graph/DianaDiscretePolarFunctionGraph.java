 ...
 */
public class DianaDiscretePolarFunctionGraph<T> extends DianaPolarFunctionGraph<T> {
	...
	public DianaDiscretePolarFunctionGraph() {
		super();
	}
	...
	@Override
	protected <U> List<FunctionSample<T, U>> retrieveSamples(DianaFunction<U> function) {
		List<FunctionSample<T, U>> returned = super.retrieveSamples(function);
		if (function.getGraphType() == DianaGraphType.SECTORS && function instanceof DianaNumericFunction) {
		...
		if (function.getDisplayLabels()) {
			...
			if (it != null) {
				while (it.hasNext()) {
					...
					Double radius = null;
					N value = null;
					if (function.getGraphType() == DianaGraphType.SECTORS) {
						radius = 0.25;
					...
					if (value != null && value.doubleValue() > 0) {
						g.drawString(label, new DianaPoint(Math.cos(angle * Math.PI / 180) * radius + 0.5,
	...
	/*@Override
	protected <N extends Number> Double getAngleExtent(T parameterValue, DianaNumericFunction<N> function) {
		if (function.getFunctionExpression() != null && function.getFunctionExpression().isSet()
				&& function.getFunctionExpression().isValid()) {
