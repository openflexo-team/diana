 ...
 */
public class DianaDiscreteSimpleFunctionGraph<T> extends DianaSimpleFunctionGraph<T> {
	...
	public DianaDiscreteSimpleFunctionGraph() {
		super();
	}
	...
	public void paintParameters(DianaShapeGraphics g) {
		...
		while (it.hasNext()) {
			...
			if (getParameterOrientation() == Orientation.HORIZONTAL) {
				g.drawString(label, new DianaPoint(getNormalizedPosition(t), 1.0 + relativeTextHeight), HorizontalTextAlignment.CENTER);
			}
			else {
				g.drawString(label, new DianaPoint(-relativeTextWidth, getNormalizedPosition(t)), HorizontalTextAlignment.CENTER);
			}
		}
	}
}
