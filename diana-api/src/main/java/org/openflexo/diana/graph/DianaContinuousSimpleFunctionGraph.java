 ...
 */
public class DianaContinuousSimpleFunctionGraph<X extends Number> extends DianaSimpleFunctionGraph<X> {
	...
	public DianaContinuousSimpleFunctionGraph(Class<X> numberClass) {
		super();
		...
		if (getParameterOrientation() == Orientation.HORIZONTAL) {
			...
			if (getDisplayMinorTicks()) {
				...
				for (double ts = minMTS; ts <= maxMTS; ts += getParameterMinorTickSpacing().doubleValue()) {
					...
					g.drawLine(new DianaPoint(nts, 0.995), new DianaPoint(nts, 1.005));
				}
			}
			...
			if (getDisplayMajorTicks() && getParameterMajorTickSpacing() != null) {
				...
				for (double ts = minMTS; ts <= maxMTS; ts += getParameterMajorTickSpacing().doubleValue()) {
					double nts = getNormalizedPosition((X) Double.valueOf(ts));
					// System.out.println("ts: on " + ts + " on nts=" + nts);
					g.drawLine(new DianaPoint(nts, 0.99), new DianaPoint(nts, 1.01));
				}
			}
			...
			if (getDisplayLabels() && getParameterMajorTickSpacing() != null) {
				...
				for (double ts = minMTS; ts <= maxMTS; ts += getParameterMajorTickSpacing().doubleValue()) {
					double nts = getNormalizedPosition((X) Double.valueOf(ts));
					g.drawString("" + ts, new DianaPoint(nts, 0.99), HorizontalTextAlignment.CENTER);
				}
