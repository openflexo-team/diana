...
public class TestDianaContinuousFunctionGraph {

	private static DianaModelFactory FACTORY;

	private static DianaContinuousSimpleFunctionGraph<Double> graph;
	private static DianaNumericFunction<Double> yFunction;
	...
	public void test1InitGraph() {
		graph = new DianaContinuousSimpleFunctionGraph<>(Double.class);
		graph.setParameter("x", Double.class);
		graph.setParameterRange(-10.0, 10.0);

		yFunction = graph.addNumericFunction("y", Double.class, new DataBinding<Double>("x*x-2*x+1"), DianaGraphType.CURVE);
	...
	// It is now possible to have multiple parameters for a DianaSimpleFunctionGraph
	/*@Test
	@TestOrder(2)
	...
	@Test
	@TestOrder(3)
	public void testEvaluation() throws TypeMismatchException, NullReferenceException, ReflectiveOperationException {
