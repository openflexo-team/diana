...
public class CreateNodeFromCenterAndSize extends Edition {
	...
	public CreateNodeFromCenterAndSize(GeomEditDrawingController controller) {
		...
		inputs.add(new ObtainDouble("Select width", 20, controller));
		inputs.add(new ObtainDouble("Select height", 20, controller));
		inputs.add(new ObtainPoint("Select center", controller));
	}

	@Override
	public void performEdition() {
		double width = ((ObtainDouble) inputs.get(0)).getInputData();
		double height = ((ObtainDouble) inputs.get(1)).getInputData();
		ObtainPoint center = (ObtainPoint) inputs.get(2);

		addConstruction(getController().getFactory().makeNodeWithCenterAndDimensionConstruction(center.getConstruction(), width, height));
