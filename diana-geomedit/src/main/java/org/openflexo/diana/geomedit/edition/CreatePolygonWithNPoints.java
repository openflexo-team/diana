...
public class CreatePolygonWithNPoints extends Edition {
	...
	public CreatePolygonWithNPoints(GeomEditDrawingController controller) {
	...
	public void performEdition() {
		...
		addConstruction(getController().getFactory().makePolygonWithNPointsConstruction(pc));
	}
	...
	public void paintEdition(JDianaDrawingGraphics graphics, DianaPoint lastMouseLocation) {
		...
		if (currentStep == 1) {
			...
			new DianaPolygon(Filling.NOT_FILLED, pts).paint(graphics);
