...
public class DianaSteppedDimensionConstraint implements Cloneable {

	private static final Logger LOGGER = Logger.getLogger(DianaSteppedDimensionConstraint.class.getPackage().getName());

	private double hStep = 1.0;
	private double vStep = 1.0;

	public DianaSteppedDimensionConstraint(double hStep, double vStep) {
		this.hStep = hStep;
		this.vStep = vStep;
	}

	public DianaDimension getNearestDimension(DianaDimension dimension, double minWidth, double maxWidth, double minHeight, double maxHeight) {
		DianaDimension d = dimension.clone();
	...
	public boolean equals(Object object) {
		if (object instanceof DianaSteppedDimensionConstraint) {
			return ((DianaSteppedDimensionConstraint) object).hStep == hStep && ((DianaSteppedDimensionConstraint) object).vStep == vStep;
		}
		return false;
	}

	@Override
	public DianaSteppedDimensionConstraint clone() {
